...
pipeline/generate.py
  ...
  - content/posts/YYYY-MM-DD-....md 로 저장 (홈 카드 + Posts 메뉴)
  - 추가로 같은 스크립트의 핵심 기술·추구 방향을 소재로 **웹 검색으로 자료를
    실제 조사**해, 인라인 참조 링크와 "작성 중 열람한 자료" 목록을 포함한
    리서치 글을 content/blog/ 에 게시 (Blog 메뉴, 별도 dedup)
        │
        ▼  변경사항 커밋 & push
...
- **밋업 정리(Posts)**: 🔗 참고 자료 섹션은 본문 주장을 뒷받침하는 공식 프로젝트
  홈페이지/문서 루트만 넣도록 프롬프트에서 강제하며, `parse_result()`가 http URL이
  없는 항목을 걸러낸다. 깊은 경로의 링크는 환각 위험 때문에 생성하지 않는다.
- **리서치 블로그(Blog)**: Claude가 **웹 검색 툴로 실제 자료를 열람**한 뒤
  검색 결과에 나온 URL만 인용하도록 강제한다(claude-code 백엔드는 WebSearch 툴,
  api 백엔드는 서버측 web_search 툴). 본문에 인라인 참조 링크가 들어가고, 글 끝에
  "작성 중 열람한 자료" 목록이 붙는다. 열람 자료가 하나도 없으면 게시하지 않는다.
